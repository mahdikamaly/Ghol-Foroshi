...
foreach ($posts as $post) {?>
               <div class="card">
                    ...
                    <a class="<?php print $post->famed == 10 ? 'danger' : 'info' ?> " href="<?= url('cart/famedpage.php?post_id='.$post->id) ?>">
                    <iconify-icon icon="il:heart"></iconify-icon>
                            <?php print $post->famed == 10 ? 'حذف' : 'افزودن'?> 
                        </a>

                        <a class="success " href="<?= url('cart/cartController.php?post_id='.$post->id) ?>">
                            <iconify-icon icon="fluent:delete-32-filled"></iconify-icon>خرید کردن
                        </a>
